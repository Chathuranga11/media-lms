    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(340px, 1fr)); gap: 1.5rem;">
        ...
        @forelse ($enrollments as $enrollment)
            @if ($enrollment->lesson)
                ...
                <x-filament::section>
                    ...
                    <x-slot name="heading">
                        <div style="display: flex; align-items: center; justify-content: space-between; gap: 1rem;">
                            ...
                            <x-filament::badge 
                                ...
                                size="sm" 
                                style="flex-shrink: 0; text-transform: uppercase; font-weight: bold;"
                            >
                    ...
                    {{-- 2. CARD BODY (Seamless List Layout) --}}
                            ...
                            @foreach ($liveClasses as $live)
                                <div style="display: flex; flex-wrap: wrap; align-items: center; gap: 1.5rem; padding: 1.5rem 0; border-bottom: 1px solid rgba(156, 163, 175, 0.2);">
                                    ...
                                    <div style="flex-shrink: 0;">
                                        <x-filament::button tag="a" href="{{ $live->zoom_url }}" target="_blank" color="success">
                                            Join Live
                                        </x-filament::button>
                                    </div>
                                ...
                                <div style="display: flex; flex-wrap: wrap; align-items: center; gap: 1.5rem; padding: 1.5rem 0; border-bottom: 1px solid rgba(156, 163, 175, 0.2);">
                                    ...
                                    <div style="display: flex; align-items: center; gap: 1rem; flex: 1; min-width: 0;">
                                        <div style="width: 3rem; height: 3rem; border-radius: 50%; background-color: rgba(14, 165, 233, 0.15); color: #0ea5e9; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                                            <x-filament::icon icon="heroicon-s-document-text" style="width: 1.5rem; height: 1.5rem;" />
                                        </div>
                                        <div style="flex: 1; min-width: 0;">
                                            <div style="font-weight: bold; font-size: 0.95rem; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                                                {{ $pdf->title ?: 'Lesson Material' }}
                                            </div>
                                            <div style="font-size: 0.75rem; opacity: 0.7; margin-top: 0.25rem;">PDF Document</div>
                                        </div>
                                    </div>

                                    <div style="flex-shrink: 0;">
                                        <x-filament::button tag="a" href="{{ route('student.download.material', ['id' => $pdf->id]) }}" target="_blank" color="info" variant="outlined">
                                            Download Notes
                                        </x-filament::button>
                                    </div>
                            ...
                            @if ($recordings->isNotEmpty())
                                @if ($isPaid)
                                    @foreach ($recordings as $rec)
                                        ...
                                        <div x-data="{ copied: false }" style="display: flex; flex-wrap: wrap; align-items: flex-start; gap: 1.5rem; padding: 1.5rem 0; border-bottom: {{ $loop->last ? 'none' : '1px solid rgba(156, 163, 175, 0.2)' }};">
                                            ...
                                            <div style="display: flex; gap: 1rem; flex: 1; min-width: 0;">
                                                <div style="width: 3rem; height: 3rem; border-radius: 50%; background-color: rgba(245, 158, 11, 0.15); color: #f59e0b; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                                                    <x-filament::icon icon="heroicon-s-play" style="width: 1.5rem; height: 1.5rem;" />
                                                </div>
                                                ...
                                                <div style="flex: 1; min-width: 0; padding-top: 0.25rem;">
                                                    ...
                                                    @if (!empty($rec->zoom_passcode) && $rec->zoom_passcode !== 'N/A')
                                                        <button @click="navigator.clipboard.writeText('{{ $rec->zoom_passcode }}'); copied = true; setTimeout(() => copied = false, 2000)" type="button" style="margin-top: 0.4rem; font-size: 0.75rem; font-family: monospace; font-weight: bold; color: #f59e0b; background: rgba(245, 158, 11, 0.1); border: 1px solid rgba(245, 158, 11, 0.2); padding: 0.2rem 0.5rem; border-radius: 0.25rem; cursor: pointer; display: inline-flex; align-items: center; gap: 0.25rem;">
                                                            <x-filament::icon icon="heroicon-o-clipboard-document" style="width: 0.9rem; height: 0.9rem;" />
                                                    ...
                                                    <div style="margin-top: 1rem; max-width: 16rem;">
                                                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.4rem;">
                                                            <span style="font-size: 0.65rem; text-transform: uppercase; font-weight: bold; opacity: 0.6; letter-spacing: 0.05em;">Views Left</span>
                                                            <span style="font-size: 0.75rem; font-weight: bold; color: {{ $isLocked ? '#ef4444' : 'inherit' }};">
                                                                {{ $remainingViews }} / {{ $maxViews }}
                                                            </span>
                                                        </div>
                                                        <div style="width: 100%; height: 6px; background-color: rgba(156, 163, 175, 0.2); border-radius: 999px; overflow: hidden;">
                                            ...
                                            <div style="flex-shrink: 0; min-width: 140px; margin-top: 0.25rem;">
                                                @if($isUnlockedNow)
                                                    <x-filament::button wire:click="$set('unlockedVideos.{{ $rec->id }}', false)" color="gray" style="width: 100%;">
                                                        Close Player
                                                    </x-filament::button>
                                                @elseif($isLocked)
                                                ...
                                                @else
                                                    <div style="position: relative; width: 100%;">
                                                        <x-filament::button wire:click="unlockVideo({{ $rec->id }})" color="warning" style="width: 100%;">
                                                            Load Replay
                                                        </x-filament::button>
                                                        <div wire:loading wire:target="unlockVideo({{ $rec->id }})" style="position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; background-color: #f59e0b; border-radius: 0.5rem; color: white; font-weight: bold; font-size: 0.9rem;">
                                                            Loading...
                                                        </div>
                                                    </div>
                                                @endif
                                            ...
                                            @if($isUnlockedNow)
                                                <div style="width: 100%; margin-top: 1rem; border-top: 1px solid rgba(156, 163, 175, 0.1); padding-top: 1.5rem;">
        ...
        @empty
            <div style="grid-column: 1 / -1; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 4rem 2rem; border-radius: 1rem; border: 1px solid rgba(156, 163, 175, 0.2); text-align: center;">
                <div style="width: 5rem; height: 5rem; border-radius: 50%; background-color: rgba(156, 163, 175, 0.1); display: flex; align-items: center; justify-content: center; margin-bottom: 1.25rem;">
                    <x-filament::icon icon="heroicon-o-academic-cap" style="width: 2.5rem; height: 2.5rem; color: #9ca3af;" />
                </div>
                ...
                <x-filament::button tag="a" href="/student/enroll" color="primary">
                    Browse Courses
                </x-filament::button>
            </div>
        @endforelse
